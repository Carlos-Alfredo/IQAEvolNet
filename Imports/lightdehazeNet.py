...
class LightDehaze_Net(nn.Module):
	...
	def __init__(self):
		...
		self.e_conv_layer1 = nn.Conv2d(1,8,1,1,0,bias=True)# self.e_conv_layer1 = nn.Conv2d(3,8,1,1,0,bias=True) 
		...
		self.e_conv_layer8 = nn.Conv2d(56,1,3,1,1,bias=True)# self.e_conv_layer8 = nn.Conv2d(56,3,3,1,1,bias=True)
	...
	def forward(self, img):
		...
		conv_layer8 = nn.functional.sigmoid(self.e_conv_layer8(concat_layer3))#self.relu(self.e_conv_layer8(concat_layer3))


		dehaze_image = self.relu(conv_layer8 * img)#self.relu((conv_layer8 * img) - conv_layer8 + 1) 
